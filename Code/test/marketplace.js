...
describe('Marketplace', function () {
  ...
  async function mintTokenAndCreateMarketItem(tokenId, price, transactionOptions, account = owner) {
    ...
    return marketplaceContract.connect(account).createMarketItem(nftContractAddress, tokenId, warrantyYear, warrantySerial, price, transactionOptions)
  }
  ...
  it('creates successive market items for the same token id by buying and selling', async function () {
    ...
    await marketplaceContract.createMarketItem(nftContractAddress, token1id, warrantyYear, warrantySerial, price, { value: listingFee })
    ...
    await marketplaceContract.connect(buyer).createMarketItem(nftContractAddress, token1id, warrantyYear, warrantySerial, price, { value: listingFee })
  ...
  it('gets latest Market Item by the token id', async function () {
    ...
    await nftContract.mintToken('')
    await marketplaceContract.createMarketItem(nftContractAddress, token1id, warrantyYear, warrantySerial, price, { value: listingFee })
    await marketplaceContract.createMarketItem(nftContractAddress, token2id, warrantyYear, warrantySerial, price, { value: listingFee })
    ...
    const marketItem = [
      ...
      ethers.constants.AddressZero,
      price,
      BigNumber.from(warrantyYear),
      BigNumber.from(warrantySerial),
      false,
      false
    ]
