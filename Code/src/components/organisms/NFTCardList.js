...
export default function NFTCardList({ nfts, setNfts, withCreateNFT }) {
  ...
  async function updateNFT(index, tokenId) {
    const updatedNFt = await mapCreatedAndOwnedTokenIdsAsMarketItems(
      marketplaceContract,
      nftContract,
      account,
    )(tokenId);
  ...
  return (
    <InfiniteScroll dataLength={nfts.length} loader={<LinearProgress />}>
      <Grid container className={classes.grid} id="grid" style={{ padding: 20 }}>
        {withCreateNFT && (
          <Grid item xs={12} sm={6} md={3} className={classes.gridItem}>
            <NFTCardCreation account={account} addNFTToList={addNFTToList} />
          </Grid>
        )}
        {nfts.map((nft, i) => (
          <Fade in={true} key={i}>
